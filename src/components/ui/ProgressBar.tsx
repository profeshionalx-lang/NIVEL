...
export default function ProgressBar({
  ...
  // When going from 0 → first point, skip the dim-layer delta effect and show bar bright
  const isFromZero = hasDelta && prevPercentage === 0;
  ...
  return (
    <div className="w-full">
      ...
      <div className="h-1.5 w-full rounded-full bg-surface-elevated relative overflow-hidden">
        {/* Dimmed delta layer — only when there's a real prev position to show */}
        {hasDelta && !isFromZero && (
          <div
        ...
        <div
          className={`absolute inset-y-0 left-0 h-full transition-all duration-500 ${barClasses[variant]}`}
          style={{
            width: `${hasDelta && !isFromZero ? prevPercentage : percentage}%`,
            borderRadius: "0 9999px 9999px 0",
          }}
        />
      </div>
